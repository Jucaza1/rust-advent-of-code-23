...
fn main() {
    // let file = File::open("./examples/day-07-part-02/input-test2.txt").expect("file not found");
...
fn process_line(s: &str) -> Row {
    ...
    state = match (&state, jokers) {
        (HandKind::High, 1) => HandKind::Pair,
        (HandKind::High, 2) => HandKind::Three,
        (HandKind::High, 3) => HandKind::Four,
        (HandKind::High, 4) => HandKind::Five,
        (HandKind::High, 5) => HandKind::Five,
        (HandKind::Pair, 1) => HandKind::Three,
        (HandKind::Pair, 2) => HandKind::Four,
        (HandKind::Pair, 3) => HandKind::Five,
        (HandKind::Three, 1) => HandKind::Four,
        (HandKind::Three, 2) => HandKind::Five,
        (HandKind::Double, 1) => HandKind::House,
        (HandKind::Four, 1) => HandKind::Five,
        (_, _) => state,
    };
    // println!("{} {:>10?}",hand,state);
    Row {
        kind: state,
        values: cards_values,
        bid,
